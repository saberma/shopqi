...
namespace :deploy do
  ...
  task :symlink_shared, roles: :app do
    %w(database.yml sunspot.yml app_secret_config.yml unicorn.conf.rb).each do |secure_file|
...
namespace :resque do # 后台任务
  ...
  task :start, roles: :app, only: { jobs: true } do
    run "cd #{current_path}; PIDFILE=#{pids_path}/resque.#{application}.pid BACKGROUND=yes QUEUE=theme_extracter,theme_exporter,solr_index,shopqi_mailer,shopqi_mailer_ship,shop_contact_us,alipay_send_goods bundle exec rake resque:work"
    run "cd #{current_path}; PIDFILE=#{pids_path}/resque-scheduler.#{application}.pid BACKGROUND=yes QUEUE=theme_extracter,theme_exporter,solr_index,shopqi_mailer,shopqi_mailer_ship,shop_contact_us,alipay_send_goods bundle exec rake resque:scheduler"
  end
