...
class ShopMailer < ActionMailer::Base
  ...
  def notify_email(email,order_id,mail_type,shop_id)
    ...
    liquid_hash.merge! 'shop' => ShopDrop.new(shop), 'is_email' => true
    ...
    body = Liquid::Template.parse(body).render(liquid_hash)
    subject = Liquid::Template.parse(email_template.title).render(liquid_hash)
    mail from: "#{shop.name} <#{shop.email}>", to: email, subject: subject, body: body, content_type: type
  end
