class SmartCollectionObserver < ActiveRecord::Observer
  ...
  def after_save(product)
    ...
    shop.smart_collections.each do |smart_collection|
      collection_product = smart_collection.products.where(product: product).first
      if smart_collection.match? product
        smart_collection.products.create(product: product, position: 0) unless collection_product
