...
class ShopMailer < ActionMailer::Base
  ...
  def notify_email(email,order_id,mail_type,shop_id)
    ...
    mail(to: email,from: Shop.find(shop_id).email, body: Liquid::Template.parse(body).render(liquid_hash.merge('shop' => ShopDrop.new(order.shop))), subject: Liquid::Template.parse(email_template.title).render(liquid_hash),content_type: type)
  end
