...
module OAuth2
  class Router
    ...
    def self.access_token(resource_owner, scopes, request, params = nil)
      params ||= request.params
      token_from_param = params[OAUTH_TOKEN]
      authorization_header ||= Rack::Auth::AbstractRequest.new(request.env)
      token_from_header = authorization_header.provided? ? authorization_header.params : nil
      access_token = token_from_param || token_from_header
